...
3	shock	electricity or shock damage
\.
...
15	Cross Slash	Performs a downwards slash. On contact with an enemy, pressing the special button allows this slash to transition into a series of follow-up slashes, spelling the kanji (きょう) ("bad luck", "misfortune").	4% (hit 1), 3% (hit 2-4), 6% (hit 5)	\N	\N
16	Climhazzard	A leaping reverse gripped slash, optionally followed with a descending reverse gripped slash	\N	\N	\N
17	Limit Charge	Holds the Buster Sword horizontally in front of himself and remains still in order to manually charge the Limit Gauge.	0	\N	\N
...
10	Blaster	Takes his Blaster out of its holster and fires at the opponent. The Blaster itself has relatively low ending lag and the projectiles have transcendent priority, which makes it good at forcing approaches at long distances.	3% (point-blank range), 2% (mid-range), 1.3% (long-range)	\N	\N
...
7	Fox Illusion	Dashes forward at a blindingly fast speed, leaving behind afterimages. Fox will launch opponents at an upwards angle on contact. It can be used as a situational recovery option.	8% (grounded version), 5% (aerial version)	\N	\N
11	Fire Fox	Engulfs himself in an aura of flame before launching himself in a fiery tackle. Since this move is fire-based, it can detonate some items like Links bombs.	1.8% (charge loop), 16% (clean dash), 10% (late dash)	\N	\N
1	Falcon Punch	Winds up and throws his iconic flaming punch, yelling "Falcon... Punch!" while doing so.	25% - 28%	massive	1
12	Reflector	Activates his Reflector, surrounding himself with a blue hexagonal energy field. All projectiles are deflected and do increased damage. The Reflector also damages nearby foes upon activation, weakly launching them at a semi-spike angle.	2% (activation), 1.4× reflected projectile	\N	\N
...
13	Blade Beam	Swings the Buster Sword upward and in front of himself in order to launch a green, crescent-shaped energy beam as a projectile.	8% (grounded clean), 6% (grounded late)	\N	\N
\.
